(function (name, context, definition) {
    ...
    var rootEl = document.documentElement || body

    //auxiliary function
    var isObject = function(value) {
      return "object" == typeof a && null !== value
    }
    ...
    _Array.prototype = {
      ...
      slice: function(start, end) {
        var result = isNumber(end) ? this._arr.slice(start, end) : this._arr.slice(start);
        return new _Array(result);
      },
      push: function(value) {
      ...
      splice: function(a, b) {
        return this._arr.splice(a, b || 1)
      },
      join: function(array) {
        return this._arr.join(array)
      },
      concat: function(array) {
        return new _Array(this._arr.concat(array))
      },
      map: function(cb) {
        var arr = this._arr;
        if (arr.map)
          return new _Array(arr.map(cb));
        for (var result = [], i = 0, len = arr.length; i < len; i += 1)
          result[i] = cb(arr[i], i, this);
        return new _Array(result)
      },
      filter: function(cb) {
        var arr = this._arr;
        if (arr.filter)
          return new _Array(arr.filter(cb));
        for (var result = [], i = 0, len = arr.length; i < len; i += 1)
          cb(arr[i], i, this) && result.push(arr[i]);
        return new _Array(result)
      },
      indexOf: function(value) {
        var arr = this._arr;
        if (!arr.indexOf) {
          for (var i = 0, len = c.length; i < len; i += 1) {
            if (arr[i] === value)
              return i;
          }
          return -1;
    ...
    _Object.prototype = {
      _each: function(process) {
        var _obj = this._obj;
        for (var key in _obj)
          _obj.hasOwnProperty(key) && process(key, _obj[key]);
        return this
      },
      yb: function() {
        var _obj = this._obj;
    ...
    function _Event(elWrapper, event) {
      this._event = event;
    ...
    _Event.prototype = {
      getClientX: function() {
        ...
        return isNumber(event.clientX) ? event.clientX : (event.changedTouches && event.changedTouches[0]).clientX
      },
      getClientY: function() {
        var event = this._event;
        return isNumber(event.clientY) ? event.clientY : (event.changedTouches && event.changedTouches[0]).clientY
      },
      preventDefault: function() {
        var event = this._event;
        event.cancelable && isFunction(event.preventDefault) ? event.preventDefault() : event.returnValue = false;
        return this;
      },
    ...
    function ElementWrapper(el) {
      this.element = "string" == typeof el ? document.createElement(el) : el
    }
    ...
    ElementWrapper.prototype = {
      ...
      setAttribute: function(attrObj) {
        var el = this.element;
        new _Object(attrObj)._each(function(key, value) {
          el.setAttribute(key, value)
        });
        return this;
      },
      removeAttribute: function(arr) {
        var el = this.element;
        new _Array(arr).map(function(value) {
          el.removeAttribute(value)
        });
        return this;
      },
      setProperty: function(props) {
        var el = this.element;
      ...
      moveTo: function(a) {
        ...
        this.appendChildTo(a);
        return this;
      },
      insertBefore: function(a) {
        a.element.parentNode.insertBefore(this.element, a.element);
        return this;
      },
      appendChild: function(a) {
        a.appendChildTo(this);
        return this;
      },
      detach: function() {
        var el = this.element;
      ...
      toggleClass: function(name) {
        ...
        var arr = new _Array(classes);
        if (-1 === arr.indexOf(GEETEST_ + name)) {
          this.addClass(name);
        } else {
      ...
      removeClass: function(name) {
        var el = this.element;
        var arr = new _Array(el.className.split(" "));
        name = GEETEST_ + name;
      ...
      replaceClass: function(target, source) {
        this.removeClass(source).addClass(target);
        return this;
      },
      _bind: function(eventGroupName, callback) {
        var self = this,
          el = self.element,
          events = self.eventGroupMap[eventGroupName],
          listener = function(event) {
            callback(new _Event(self, event))
          };
        return new _Array(events).map(function(value) {
          if (document.addEventListener)
            el.addEventListener(value, listener);
          else if (document.attachEvent)
            el.attachEvent("on" + value, listener);
          else {
            var eventFunc = el["on" + value];
            el["on" + value] = function(event) {
              callback(new _Event(self, event)),
                "function" == typeof eventFunc && eventFunc.call(this, event)
            }
          }
        }), {
          _unbind: function() {
            new _Array(events).map(function(value) {
              document.removeEventListener ? el.removeEventListener(value, listener) : document.detachEvent ? el.detachEvent("on" + value, listener) : el["on" + value] = null
            })
          }
        }
      },
      bind: function(eventGroupName, callback) {
        this._bind(eventGroupName, callback);
        return this;
      },
      ...
      getStyleValue: function(name) {
        var el = this.element;
        return window.getComputedStyle ? window.getComputedStyle(el)[name] : el.currentStyle[name]
      },
      getTotalOffsetLeft: function() {
        for (var thisEl = this.element, totalOffsetLeft = thisEl.offsetLeft, currEl = thisEl.offsetParent; null !== currEl;) {
          totalOffsetLeft += currEl.offsetLeft;
          currEl = currEl.offsetParent;
        }
        return totalOffsetLeft;
      },
      getTotalOffsetTop: function() {
        for (var thisEl = this.element, totalOffsetTop = thisEl.offsetTop, currEl = thisEl.offsetParent; null !== currEl;) {
          totalOffsetTop += currEl.offsetTop;
          currEl = currEl.offsetParent;
        }
        return totalOffsetTop;
      }
    }
    ElementWrapper.$ = function(selector) {
      var el;
    ...
    Tracker.prototype = {
      ud: /*msie6 || msie7 ? 150 :*/ 300,
      init: function() {
        var self = this;
        new ElementWrapper(document).bind("move", function(event) {
            self.syncScroll(),
              self.posX = event.getClientX(),
              self.posY = event.getClientY(),
              self.addTrack("move", self.posX, self.posY)
          }).bind("down", function(event) {
            self.syncScroll(),
              self.posX = event.getClientX(),
              self.posY = event.getClientY(),
              self.addTrack("down", self.posX, self.posY);
            var len = self._trackArray.length;
            self._trackArray[len - 2] && "move" === self._trackArray[len - 2][0] && self._trackArray[len - 3] && "up" === self._trackArray[len - 3][0] && self.deleteTrack(len - 2)
          }).bind("up", function(event) {
            self.syncScroll(),
              self.posX = event.getClientX(),
              self.posY = event.getClientY(),
              self.addTrack("up", self.posX, self.posY);
            var len = self._trackArray.length;
            self._trackArray[len - 2] && "move" === self._trackArray[len - 2][0] && self._trackArray[len - 3] && "down" === self._trackArray[len - 3][0] && self.deleteTrack(len - 2)
          }),
          new ElementWrapper(window).bind("scroll", function() {
            var c = "pageXOffset" in window,
              d = c ? window.pageXOffset : document.body.scrollLeft,
              e = c ? window.pageYOffset : document.body.scrollTop;
            self.posX = d - self.scrollLeft + self.posX,
              self.posY = e - self.scrollTop + self.posY,
              self.addTrack("scroll", d - self.scrollLeft + self.posX, e - self.scrollTop + self.posY),
              self.syncScroll()
          }).bind("focus", function() {
            self.addTrack("focus")
          }).bind("blur", function() {
            ...
            self.addTrack("unload")
          })
      },
      syncScroll: function() {
        var exist = "pageXOffset" in window;
        var left = exist ? window.pageXOffset : document.body.scrollLeft;
        var top = exist ? window.pageYOffset : document.body.scrollTop;
      ...
      addTrack: function() {
        var lastX, lastY, lastTime = getCurrentTime();
        return function(eventGroupName, x, y) {
          var currentTime = getCurrentTime(),
            arr = this._trackArray;
          if (new _Array(["move", "down", "up", "scroll"]).indexOf(eventGroupName) > -1) {
            if ("move" === eventGroupName) {
              if (x === lastX && y === lastY || lastTime === currentTime)
                return;
              lastX = x;
              lastY = y;
              lastTime = currentTime;
            }
            arr.push([eventGroupName, this.toInt16(x), this.toInt16(y), currentTime])
          } else {
            arr.push([eventGroupName, currentTime]);
          }
          return this;
        }
      }(),
      deleteTrack: function(index) {
        this._trackArray.splice(index, 1)
      },
      calcDiff: function(trackArr) {
        var lastX = 0,
          lastY = 0,
          result = [],
          self = this,
        ...
        if (trackArr.length <= 0)
          return [];
        for (var len = trackArr.length, i = len < this.ud ? 0 : len - this.ud; i < len; i += 1) {
          var track = trackArr[i],
            eventName = track[0];
          if (new _Array(["down", "move", "up", "scroll"]).indexOf(eventName) > -1) {
            result.push([eventName, [track[1] - lastX, track[2] - lastY], self.toInt16(lastTime ? track[3] - lastTime : lastTime)]);
            lastX = track[1];
            lastY = track[2];
            lastTime = track[3];
          } else {
            if (new _Array(["blur", "focus", "unload"]).indexOf(eventName) > -1) {
              result.push([eventName, self.toInt16(lastTime ? track[1] - lastTime : lastTime)]);
              lastTime = track[1];
            }
          }
        }
      ...
      encode: function(b) {
        function c(b) {
          for (var result = "", len = b.length / 6, i = 0; i < len; i += 1) {
            result += j.charAt(window.parseInt(b.slice(6 * i, 6 * (i + 1)), 2));
          }
          return result;
        }
        // 将数字转成定长的二进制字符串，不足位数，前补零
        function d(num, totalLen) {
          for (var binaryStr = num.toString(2), len = binaryStr.length, str = "", pos = len + 1; pos <= totalLen; pos += 1)
            str += "0";
          return binaryStr = str + binaryStr;
        }

        function e(a, b) {
          for (var arr = [], i = 0, len = a.length; i < len; i += 1)
            arr.push(b(a[i]));
          return arr
        }
        ...
        function g(a) {
          ...
          for (var len = a.length, c = 0, d = []; c < len;) {
        ...
        return function(a) {
          for (var b = [], e = [], f = [], g = [], h = 0, len = a.length; h < len; h += 1) {
            var k = a[h],
              m = k.length;
            b.push(k[0]),
              e.push(2 === m ? k[1] : k[2]),
      ...
      toInt16: function(value) {
        return "number" != typeof value ? value : (value > 32767 ? value = 32767 : value < -32767 && (value = -32767),
          Math.round(value))
      },
      Jc: function() {
        return this.encode(this.calcDiff(this._trackArray)).length
      },
      encodedDiff: function() {
        var b = this._trackArray;
        this._trackArray = [];
        return this.encode(this.calcDiff(b))
      },
    ...
    //ClickTracker implementation
    var ClickTracker = function(){
      var self = this;
      self.elTagArray = [];
      self.index = 0;
      self.elIndexMap = [];
      self.nodeType = document.body.nodeType;
      new ElementWrapper(document).bind("click", function(eventWrapper) {
        self.addClick(eventWrapper._event);
      });
    }

    ClickTracker.prototype = {
      /* Jc: function() {
         return this.elTagArray.join("|").length
       },*/
      extractResult: function() {
        var result = this.elTagArray || [];
        this.elTagArray = [];
        this.index = 0;
        this.elIndexMap = [];
        /* (msie6 || msie7) && (b = b.slice(0, 10));*/
        return result.join("|");
      },
      addClick: function(event) {
        var elTagArray = this.elTagArray,
          target = event.target || event.srcElement;
        if (target && target.nodeType === this.nodeType) {
          for (var elIndex = null, i = this.elIndexMap.length - 1; i >= 0; i--)
            if (this.elIndexMap[i].el === target) {
              elIndex = this.elIndexMap[i];
              break;
            }
          if (elIndex) {
            elTagArray.push(elIndex.el.tagName + "_" + elIndex.index);
          } else {
            this.elIndexMap.push({
              el: target,
              index: this.index
            });
            elTagArray.push(target.tagName + "_" + this.index);
            this.index++;
          }
        } else
          c.push("");
